...
class ContactEditor {
    ...
    async init() {
        console.log('🚀 ContactEditor: Initializing...');
    ...
    setupEventListeners() {
        ...
        document.getElementById('preview-btn')?.addEventListener('click', () => {
            ...
            enableSectionEditButtons() {
                ...
                if (!preview) {
                    console.error('contact-preview not found');
    ...
    async saveChanges() {
        ...
        try {
                    const response = await fetch('/api/contact-content/update', {
                method: 'POST',
                headers: {
...
// Make ContactEditor available globally
window.ContactEditor = ContactEditor;

// Function to initialize editor
function initContactEditor() {
    console.log('🔍 Looking for contact-preview...');
    const preview = document.getElementById('contact-preview');
    console.log('📦 contact-preview element:', preview);
    
    if (preview) {
        console.log('✅ Found contact-preview! Initializing ContactEditor...');
        window.aboutEditor = new ContactEditor();
        return true;
    } else {
        console.warn('⚠️ contact-preview not found yet');
        return false;
    }
}
...
function tryInit() {
    attempts++;
    console.log(`🔄 Attempt ${attempts}/${maxAttempts} to find contact-preview...`);
    
    if (initContactEditor()) {
    ...
    if (attempts < maxAttempts) {
        ...
        console.error('❌ Failed to find contact-preview after all attempts');
        // Try one more time with querySelector as fallback
        const preview = document.querySelector('[id="contact-preview"]');
        if (preview) {
